...
public class Road{
	...
	void AutoSetAnchorControlPoints(int anchorIndex) {
		...
		float[] neihbourDistances = new float[2];

		if (anchorIndex - 3 >= 0 || isClosed) {
			Vector2 offset = points[LoopIndex(anchorIndex - 3)] - anchorPos;
			dir += offset.normalized;
			neihbourDistances[0] = offset.magnitude;
		}
		if (anchorIndex + 3 >= 0 || isClosed) {
			Vector2 offset = points[LoopIndex(anchorIndex + 3)] - anchorPos;
			dir -= offset.normalized;
			neihbourDistances[1] = -offset.magnitude;
		}
		...
		for (int i = 0; i < 2; i++) {
			...
			if (controlIndex >= 0 && controlIndex < points.Count || isClosed) {
				points[LoopIndex(controlIndex)] = anchorPos + dir * neihbourDistances[i] * .5f;
			}
		}
	}
